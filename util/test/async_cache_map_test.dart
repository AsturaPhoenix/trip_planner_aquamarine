...
void main() {
  group('can add sync futures', () {
    test('for single', () async {
      final cache = AsyncCacheMap<int, int>.single();
      expect(await cache.computeIfAbsent(0, () => Future.value(0)), 0);
    });

    test('for ephemeral', () async {
      int calls = 0;
      final cache = AsyncCacheMap<int, int>.ephemeral();
  ...
  group('replaced operation does not remove replacement', () {
    test('for single', () async {
      final cache = AsyncCacheMap<int, void>.single();
      ...
      try {
        await oldFuture;
        fail('Expected exception from oldFuture');
      } on Exception {}
    ...
    test('for ephemeral', () async {
      final cache = AsyncCacheMap<int, void>.ephemeral();
